...
<c:if test="${not empty club}">
    ...
    <c:choose>
       <c:when test="${isMember}">
           <button type="button" class="btn btn-outline-primary1" disabled>클럽 참여중</button>
       </c:when>
	    <c:when test="${club.total == club.nowTotal}">
	        <button type="button" class="btn btn-outline-primary1" disabled>클럽 참여불가</button>
	    </c:when>
	    <c:otherwise>
	        <form action="join" method="post">
	            <input type="text" name="no" value="${club.no}" hidden>
	            <input type="text" name="loginUser" value="${loginUser.no}" hidden>
	            <input class="btn btn-outline-primary1" value="클럽 참여" onclick="result1()" style="font-size: 11px; position: absolute; right: 5%; top: 4%;">
	        </form>
	    </c:otherwise>
    </c:choose>
    <!--클럽신고 관련-->
...
<%--            번호: --%>
            ...
            <c:if test="${loginUser.no == club.writer.no}">
                <input type='text' name='title' value='${club.title}' style="width: 50%; background-color: #f8f8f8; font-size: 27px; border: #f8f8f8; font-weight: bold; position: absolute; top: 35%; left: 18%;"><br>
                <textarea name='content' rows='3' cols='170' style="background-color: #f8f8f8; border: #000; font-size: 15px; position: absolute; top: 42%; left: 18%;">${club.content}</textarea><br>
            </c:if>

            <c:if test="${loginUser.no != club.writer.no}">
                <input type='text' name='title' value='${club.title}' style="width: 50%; background-color: #f8f8f8; font-size: 27px; border: #f8f8f8; font-weight: bold; position: absolute; top: 35%; left: 18%;" disabled><br>
                <textarea name='content' rows='3' cols='170' style="background-color: #f8f8f8; border: #000; font-size: 15px; position: absolute; top: 42%; left: 18%;" disabled >${club.content}</textarea>
            </c:if>
            ...
            <br>
            <p style="font-size: 13px; color: f8f8f8; position: absolute; top: 62%; left: 18%;">도착지: <input type='text' name='arrive' value='${club.arrive}' disabled style="border: #f8f8f8; width: 100px;">
            ...
            인원: ${club.nowTotal} / ${club.total}</p> <br>
            ...
            <c:if test="${not empty loginUser and loginUser.no == club.writer.no or loginUser.power == 1 }">
                ...
                <tr>
                    <td colspan='2'>
                        <input type='submit' class=" btn btn-outline-primary1" value='변경'>
                        <input onclick="del()" class="btn btn-primary1" value="삭제" style="margin-left: 84%; margin-top: 7%">
                    </td>
                </tr>
                </tfoot>
            </c:if>
